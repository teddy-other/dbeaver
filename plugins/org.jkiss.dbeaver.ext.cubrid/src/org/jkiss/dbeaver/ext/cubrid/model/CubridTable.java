...
//                    	}
//                    } else 
                    if (sqlQuery.contains("autocommit off")) {
...
//    			            }
    		            	...
    		            	if (dbStat.getQueryString().contains("limit")) {
    		            		log.info(String.format("limit sqlQuery %s", dbStat.getQueryString()));
    		            	}
    		            }
                    }
		        } catch (Exception e) {
		        	//if (sqlQuery.contains("select")) {
			        	log.error(String.format("test fail file %s" ,file));
			        	log.error(String.format("test fail select query : %s" ,sqlQuery));
			        	if (dbStat != null) {
			        		//if (dbStat.getQueryString().contains("limit")) {
			        			log.error(String.format("test limit fail sqlQuery : %s", dbStat.getQueryString()));
			        		//}
			        	}
			        	log.error(e);
		        	//}
    ...
    public static void processLimitTest(LinkedHashMap<String, List<String>> sqlList) {
        ...
        Pattern commentEpattern = Pattern.compile("(\\*/)");
        StringBuilder strBuilder = new StringBuilder();
        File path = new File("d:\\cubrid-testcases\\medium");
        //File path = new File("d:\\cubrid-testcases\\temp");
        ...
        for (File f : sqlFiles) {
            ...
            try {
                ...
                while (scanner.hasNextLine()) {
                    ...
                    str = str.trim();
                    int checkIndex = str.indexOf("--");
                    if (checkIndex > 1) {
                    	str = str.substring(0, checkIndex);
                    }
                    boolean ret = pattern.matcher(str).find();
                    
                    if (ret) {
                    	Matcher sMatcher = commentSpattern.matcher(str);
                    	Matcher eMatcher = commentEpattern.matcher(str);
                    	boolean sCommentExist = sMatcher.find();
                    	boolean eCommentExist = eMatcher.find();
	                    if (sCommentExist && eCommentExist) {
	                    	int sCommentIndex = sMatcher.start();
	                    	int eCommentIndex = eMatcher.start();
	                    	skipParsing = false;
	                    	if (sCommentIndex > 0) {
	                    		//System.out.println("old str : " + str);
	                    		String deleteChar = str.substring(sCommentIndex, eCommentIndex + 2);
	                    		//System.out.println("old deleteChar : " + deleteChar);
	                    		str = str.replace(deleteChar, "");
	                    		//System.out.println("new str : " + str);
	                    		ret = false;
	                    	}
	                    } else if (sCommentExist) {
	                    	skipParsing = true;
	                    } else if (eCommentExist) {
	                    	skipParsing = false;
	                    }
                    } else {
                    	if (skipParsing) {
